...
esptool.py --chip esp32s3 merge_bin --flash_mode dio --flash_size 16MB --flash_freq 80m 0x0 build\bootloader\bootloader.bin 0x8000 build\partition_table\partition-table.bin 0x9000 build\nvs_4.8T.bin 0x10000 build\esp-miner.bin 0x410000 build\www.bin 0xf10000 build\ota_data_initial.bin -o release\JingleMiner4.8T_complete.bin
...
REM 验证文件是否生成
set "files_ok=1"
if not exist "release\JingleMiner1.2T_complete.bin" (
    echo ❌ release\JingleMiner1.2T_complete.bin 未找到
    set "files_ok=0"
) else (
    echo ✅ release\JingleMiner1.2T_complete.bin 已生成
)

if not exist "release\JingleMiner4.8T_complete.bin" (
    echo ❌ release\JingleMiner4.8T_complete.bin 未找到
    set "files_ok=0"
) else (
    echo ✅ release\JingleMiner4.8T_complete.bin 已生成
)
...
if exist release\JingleMiner1.2T_complete.bin (
    for %%A in (release\JingleMiner1.2T_complete.bin) do echo   1.2T版本: %%~zA 字节 (%%~nxA)
)
if exist release\JingleMiner4.8T_complete.bin (
    for %%A in (release\JingleMiner4.8T_complete.bin) do echo   4.8T版本: %%~zA 字节 (%%~nxA)
)
